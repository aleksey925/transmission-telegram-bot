...
def get_torrents_inline(update: telegram.Update, context: CallbackContext[Any, Any, Any]):
    ...
    if len(callback) == 3 and callback[2] == "reload":
        try:
            query.edit_message_text(text=torrent_list, reply_markup=keyboard, parse_mode="MarkdownV2")
...
def run():
    ...
    updater.dispatcher.add_handler(CallbackQueryHandler(torrent_files_inline, pattern="torrentsfiles\\_*"))
